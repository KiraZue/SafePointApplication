...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { name: 'Emergency Report', path: '/reports', style: 'bg-[#2f4863] text-white font-bold cursor-pointer hover:bg-lime-500 hover:text-black transition-colors', showBadge: true },
  ];
            ...
            {menuItems.map((item) => (
              <li key={item.path}>
                <Link
                  ...
                  className={`flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-200 relative ${item.style} ${
